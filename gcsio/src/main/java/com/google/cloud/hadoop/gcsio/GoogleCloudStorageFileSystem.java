...
public class GoogleCloudStorageFileSystem {
  ...
  public List<FileInfo> listFileInfo(URI path) throws IOException {
    ...
    StorageResourceId dirId =
        StorageResourceId.fromUriPath(UriPaths.toDirectory(path), /* allowEmptyObjectName= */ true);
    ...
    if (dirItemInfos.get(0).getObjectName() != null
        && dirItemInfos.get(0).getObjectName().equals(dirId.getObjectName())) {
  ...
  private GoogleCloudStorageItemInfo getFileInfoInternal(
    ...
    try {
      ...
      try {
        return listDirFuture.get().isEmpty()
                || (listDirFuture.get().get(0).implicitDirectory() && !inferImplicitDirectories)
            ? GoogleCloudStorageItemInfo.createNotFound(resourceId)
            : listDirFuture.get().get(0);
